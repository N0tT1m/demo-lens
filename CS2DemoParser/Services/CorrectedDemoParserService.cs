...
public class CorrectedDemoParserService
{
    ...
    {
        // Packet events for server info and voice data
        demo.PacketEvents.SvcServerInfo += OnSvcServerInfo;
        // demo.PacketEvents.SvcVoiceData += OnSvcVoiceData; // Commented out - VoiceCommunication model doesn't have required properties
        ...
        try
        {
            // Grenade inventory tracking (proven to work in demofile-net examples)
            demo.EntityEvents.CCSPlayerPawn.AddCollectionChangeCallback(
                pawn => pawn.Grenades,
                (pawn, oldGrenades, newGrenades) => OnPlayerGrenadeInventoryChanged(pawn, oldGrenades, newGrenades)
            );

            // Weapon inventory tracking
            demo.EntityEvents.CCSPlayerPawn.AddCollectionChangeCallback(
                pawn => pawn.WeaponServices?.MyWeapons,
                (pawn, oldWeapons, newWeapons) => OnPlayerWeaponInventoryChanged(pawn, oldWeapons, newWeapons)
            );
        }
        catch (Exception ex)
        {
        ...
        // User Message Events - All available events for comprehensive tracking
    ...
    private async Task CalculateAndSavePlayerMatchStats()
    {
        ...
        foreach (var player in _players.Values)
        {
            // Try to get the player controller to access built-in stats
            var playerController = _demo?.Players.FirstOrDefault(p => p.SteamID == player.SteamId);
            var builtInStats = playerController?.ActionTrackingServices?.MatchStats;
            ...
            var matchStats = new Models.PlayerMatchStats
            {
                ...
                // Use built-in stats if available, otherwise use manually tracked stats
                Kills = builtInStats?.Kills ?? playerKills.Count,
                Deaths = builtInStats?.Deaths ?? playerDeaths.Count,
                Assists = builtInStats?.Assists ?? playerAssists.Count,
                HeadshotKills = builtInStats?.HeadShotKills ?? playerKills.Count(k => k.IsHeadshot),
                TotalDamageDealt = builtInStats?.Damage ?? playerDamages.Sum(d => d.DamageAmount),
    ...
    #region User Message Event Handlers
    ...
    {
        ProcessAdvancedUserMessage("close_caption_direct", null, $"Caption direct: {e.Hash}");
    }

    private void OnSendAudioUserMessage(CCSUsrMsg_SendAudio e)
    {
        ProcessAdvancedUserMessage("send_audio", null, "Send audio: Event data");
    }

    private void OnRawAudioUserMessage(CCSUsrMsg_RawAudio e)
    {
        ProcessAdvancedUserMessage("raw_audio", null, $"Raw audio: Pitch {e.Pitch}");
    }
    ...
    {
        ProcessAdvancedUserMessage("key_hint_text", null, "Key hint: Event data");
    }

    private void OnProcessSpottedEntityUpdateUserMessage(CCSUsrMsg_ProcessSpottedEntityUpdate e)
    {
        ProcessAdvancedUserMessage("process_spotted_entity_update", null, $"Spotted entity update: {e.NewUpdate}");
    }

    private void OnReloadEffectUserMessage(CCSUsrMsg_ReloadEffect e)
    {
        ProcessAdvancedUserMessage("reload_effect", null, $"Reload effect: Entity {e.Entidx}");
    }
    ...
    {
        ProcessAdvancedUserMessage("xrank_get", null, $"XRank get: Mode {e.ModeIdx}");
    }

    private void OnXRankUpdUserMessage(CCSUsrMsg_XRankUpd e)
    {
        ProcessAdvancedUserMessage("xrank_upd", null, $"XRank update: Mode {e.ModeIdx}");
    }

    private void OnCallVoteFailedUserMessage(CCSUsrMsg_CallVoteFailed e)
    {
        ProcessAdvancedUserMessage("call_vote_failed", null, $"Call vote failed: {e.Reason}");
    }

    private void OnVoteStartUserMessage(CCSUsrMsg_VoteStart e)
    {
        ProcessAdvancedUserMessage("vote_start", null, $"Vote start: {e.VoteType}");
    }

    private void OnVotePassUserMessage(CCSUsrMsg_VotePass e)
    {
        ProcessAdvancedUserMessage("vote_pass", null, $"Vote passed: {e.VoteType}");
    }

    private void OnVoteFailedUserMessage(CCSUsrMsg_VoteFailed e)
    {
        ProcessAdvancedUserMessage("vote_failed", null, $"Vote failed: {e.Reason}");
    }
    ...
    {
        ProcessAdvancedUserMessage("server_rank_reveal_all", null, "Server rank reveal: Event data");
    }

    private void OnSendLastKillerDamageToClientUserMessage(CCSUsrMsg_SendLastKillerDamageToClient e)
    {
        ProcessAdvancedUserMessage("send_last_killer_damage", null, "Last killer damage: Event data");
    }

    private void OnServerRankUpdateUserMessage(CCSUsrMsg_ServerRankUpdate e)
    {
        ProcessAdvancedUserMessage("server_rank_update", null, "Server rank update: Event data");
    }

    private void OnItemPickupUserMessage(CCSUsrMsg_ItemPickup e)
    {
        ProcessAdvancedUserMessage("item_pickup", null, $"Item pickup: {e.Item}");
    }

    private void OnShowMenuUserMessage(CCSUsrMsg_ShowMenu e)
    {
        ProcessAdvancedUserMessage("show_menu", null, $"Show menu: {e.BitsValidSlots} slots");
    }
    ...
    {
        ProcessAdvancedUserMessage("ammo_denied", null, "Ammo denied: Event data");
    }
    ...
    {
        ProcessAdvancedUserMessage("match_stats_update", null, "Match stats update");
    }

    private void OnItemDropUserMessage(CCSUsrMsg_ItemDrop e)
    {
        ProcessAdvancedUserMessage("item_drop", null, $"Item drop: {e.Itemid}");
    }

    private void OnGlowPropTurnOffUserMessage(CCSUsrMsg_GlowPropTurnOff e)
    {
        ProcessAdvancedUserMessage("glow_prop_turn_off", null, $"Glow prop off: Entity {e.Entidx}");
    }
    ...
    {
        ProcessAdvancedUserMessage("round_backup_filenames", null, $"Round backup: {e.Count} files");
    }

    private void OnSendPlayerItemFoundUserMessage(CCSUsrMsg_SendPlayerItemFound e)
    {
        ProcessAdvancedUserMessage("send_player_item_found", null, "Player item found: Event data");
    }

    private void OnReportHitUserMessage(CCSUsrMsg_ReportHit e)
    {
        ProcessAdvancedUserMessage("report_hit", null, "Report hit: Event data");
    }
    ...
    {
        ProcessAdvancedUserMessage("player_decal_digital_signature", null, $"Decal signature: {e.Data}");
    }

    private void OnWeaponSoundUserMessage(CCSUsrMsg_WeaponSound e)
    {
        ProcessAdvancedUserMessage("weapon_sound", null, $"Weapon sound: Entity {e.Entidx}");
    }

    private void OnUpdateScreenHealthBarUserMessage(CCSUsrMsg_UpdateScreenHealthBar e)
    {
        ProcessAdvancedUserMessage("update_screen_health_bar", null, $"Health bar: Entity {e.Entidx}");
    }

    private void OnEntityOutlineHighlightUserMessage(CCSUsrMsg_EntityOutlineHighlight e)
    {
        ProcessAdvancedUserMessage("entity_outline_highlight", null, $"Entity outline: {e.Entidx}");
    }
    ...
    {
        ProcessAdvancedUserMessage("survival_stats", null, "Survival stats: Event data");
    }

    private void OnEndOfMatchAllPlayersDataUserMessage(CCSUsrMsg_EndOfMatchAllPlayersData e)
    {
        ProcessAdvancedUserMessage("end_of_match_all_players_data", null, "End of match: Event data");
    }

    private void OnPostRoundDamageReportUserMessage(CCSUsrMsg_PostRoundDamageReport e)
    {
        ProcessAdvancedUserMessage("post_round_damage_report", null, "Post round damage: Event data");
    }
    ...
    private void OnSvcServerInfo(CSVCMsg_ServerInfo e)
    {
        _logger.LogInformation("Server Info: Map={MapName}, TickInterval={TickInterval}, TickRate={TickRate}",
            e.MapName, e.TickInterval, 1 / e.TickInterval);
    }

    // Commented out - VoiceCommunication model doesn't have required properties
    //private void OnSvcVoiceData(CSVCMsg_VoiceData e)
    //{
    //    if (_demo == null || _currentDemoFile == null) return;

    //    var player = _demo.GetPlayerBySteamId(e.Xuid);
    ...
    private void OnPlayerGrenadeInventoryChanged(CCSPlayerPawn pawn, IReadOnlyList<CBaseCSGrenade> oldGrenades, IReadOnlyList<CBaseCSGrenade> newGrenades)
    {
        if (_demo == null || _currentDemoFile == null || pawn.Controller == null) return;

        var player = GetOrCreatePlayer(pawn.Controller);

        var inventoryChange = new Models.Equipment
        {
            DemoFileId = _currentDemoFile.Id,
            Player = player,
            Tick = _demo.CurrentDemoTick.Value,
        ...
        var playerPos = pawn.Origin;
        inventoryChange.PositionX = (decimal)playerPos.X;
        inventoryChange.PositionY = (decimal)playerPos.Y;
        inventoryChange.PositionZ = (decimal)playerPos.Z;

        _equipment.Add(inventoryChange);
    }

    private void OnPlayerWeaponInventoryChanged(CCSPlayerPawn pawn, IReadOnlyList<CHandle<CBasePlayerWeapon, CsDemoParser>>? oldWeapons, IReadOnlyList<CHandle<CBasePlayerWeapon, CsDemoParser>>? newWeapons)
    {
        if (_demo == null || _currentDemoFile == null || pawn.Controller == null || newWeapons == null) return;
